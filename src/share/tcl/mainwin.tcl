...
$toolmenu add command -label  "AFM-SEM" -underline 1 \
    -command "show.afm_sem_win"
}

lappend stream_only_controls [list $toolmenu entryconfigure "Replay Control"]

#### Virus menu #############################
#### Interface for Education Research Group
#### Allows interaction with three different streamfiles
#### that show the adenovirus and tobacco mosaic virus in three experiments
#### Cut, Poke, Push. Each experiment streamfile has several outcomes
#### the outcomes are viewed by clickingon the 'push' 'cut' or 'poke' buttons
#### 
set virus_streamfile_active 0

proc setup_virus_streamfile_menu { nm el op } {
    global virus_streamfile_active
    if { $virus_streamfile_active } {
        set virusmenu .menu.virus
        menu $virusmenu -tearoff 0
        .menu add cascade -label "Virus" -menu $virusmenu -underline 1

        $virusmenu add command -label  "Cut" -command cut_virus
        $virusmenu add command -label "Poke" -command poke_virus
        $virusmenu add command -label "Push" -command push_virus
    }
}
trace variable virus_streamfile_active w setup_virus_streamfile_menu
...
source [file join ${tcl_script_dir} nanoed_streamfiles.tcl]
...
# AFM SEM calibration
source [file join ${tcl_script_dir} afm_sem.tcl]
}
