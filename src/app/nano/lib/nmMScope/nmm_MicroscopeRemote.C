...
// ../../../../../../vrpn/util/gen_rpc/gen_vrpn_rpc.pl nmm_MicroscopeRemote.Cdef
...
*/
  ...
  markTypeAsSafe(d_QueryScanRange_type);
    connection->register_handler(d_HelloMessage_type,
                    handle_HelloMessage,
                    this);
    connection->register_handler(d_ScanrateParameter_type,
                    handle_ScanrateParameter,
                    this);
    connection->register_handler(d_PidParameters_type,
                    handle_PidParameters,
                    this);
    connection->register_handler(d_ReportGridSize_type,
                    handle_ReportGridSize,
                    this);
    connection->register_handler(d_SetRegionClipped_type,
                    handle_SetRegionClipped,
                    this);
    connection->register_handler(d_ScanRange_type,
                    handle_ScanRange,
                    this);
    connection->register_handler(d_ReportScanAngle_type,
                    handle_ReportScanAngle,
                    this);
    connection->register_handler(d_ReportSlowScan_type,
                    handle_ReportSlowScan,
                    this);
    connection->register_handler(d_Scanning_type,
                    handle_Scanning,
                    this);
    connection->register_handler(d_ScanDataset_type,
                    handle_ScanDataset,
                    this);
    connection->register_handler(d_WindowLineData_type,
                    handle_WindowLineData,
                    this);
    connection->register_handler(d_PointDataset_type,
                    handle_PointDataset,
                    this);
    connection->register_handler(d_ForceCurveData_type,
                    handle_ForceCurveData,
                    this);
    connection->register_handler(d_RelaxSet_type,
                    handle_RelaxSet,
                    this);
    connection->register_handler(d_StartingToRelax_type,
                    handle_StartingToRelax,
                    this);
    connection->register_handler(d_TopoFileHeader_type,
                    handle_TopoFileHeader,
                    this);
    connection->register_handler(d_SuspendCommands_type,
                    handle_SuspendCommands,
                    this);
    connection->register_handler(d_ResumeCommands_type,
                    handle_ResumeCommands,
                    this);
    connection->register_handler(d_MaxSetpointExceeded_type,
                    handle_MaxSetpointExceeded,
                    this);
    connection->register_handler(d_InScanlineMode_type,
                    handle_InScanlineMode,
                    this);
    connection->register_handler(d_ScanlineData_type,
                    handle_ScanlineData,
                    this);
    connection->register_handler(d_BeginFeelTo_type,
                    handle_BeginFeelTo,
                    this);
    connection->register_handler(d_EndFeelTo_type,
                    handle_EndFeelTo,
                    this);
    connection->register_handler(d_InContactMode_type,
                    handle_InContactMode,
                    this);
    connection->register_handler(d_InGuardedScanMode_type,
                    handle_InGuardedScanMode,
                    this);
    connection->register_handler(d_InTappingMode_type,
                    handle_InTappingMode,
                    this);
    connection->register_handler(d_InOscillatingMode_type,
                    handle_InOscillatingMode,
                    this);
    connection->register_handler(d_InSewingStyle_type,
                    handle_InSewingStyle,
                    this);
    connection->register_handler(d_InSpectroscopyMode_type,
                    handle_InSpectroscopyMode,
                    this);
    connection->register_handler(d_InDirectZControl_type,
                    handle_InDirectZControl,
                    this);
    connection->register_handler(d_InModMode_type,
                    handle_InModMode,
                    this);
    connection->register_handler(d_InImgMode_type,
                    handle_InImgMode,
                    this);
...
char * nmm_Microscope_Remote::encode_PointResultData (
      int * len,
      vrpn_float32 x,
      vrpn_float32 y,
      vrpn_int32 sec,
  ...
  if (!len) return NULL;
  *len =
     (sizeof(vrpn_float32) + sizeof(vrpn_float32) + sizeof(vrpn_int32) + sizeof(vrpn_int32) + sizeof(vrpn_int32) + sizeof(vrpn_float32) * reports);
  msgbuf = new char [*len];
  if (!msgbuf) {
    fprintf(stderr, "encode_PointResultData: Out of memory.\n");
    *len = 0;
  } else {
    *mptr = msgbuf;
    *mlen = *len;
    ...
    for (int lv_1 = 0; lv_1 < reports; lv_1++ ) {
      vrpn_buffer(mptr, mlen, data[lv_1]);
    }
  }
  return msgbuf;
}

int nmm_Microscope_Remote::decode_PointResultData (
      const char ** buffer,
      vrpn_float32 (*x),
      vrpn_float32 (*y),
      vrpn_int32 (*sec),
  ...
  if (vrpn_unbuffer(buffer, sec)) return -1;
  if (vrpn_unbuffer(buffer, usec)) return -1;
  if (vrpn_unbuffer(buffer, reports)) return -1;
  (*data) = new vrpn_float32  [*reports];
  for (int lv_1 = 0; lv_1 < *reports ; lv_1++ ) {
    if (vrpn_unbuffer(buffer, &((*data)[lv_1]))) return -1;
  }
  return 0;
}
...
int nmm_Microscope_Remote::handle_PointResultData (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_float32 x;
  vrpn_float32 y;
  vrpn_int32 sec;
  vrpn_int32 usec;
  ...
  me->decode_PointResultData ( &buffer, &x, &y, &sec, &usec, &reports, &data );
  me->RcvPointResultData ( x, y, sec, usec, reports, data );
  delete [] data;
  return 0;
}

int nmm_Microscope_Remote::handle_HelloMessage (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  char nm[4];
  ...
  me->decode_HelloMessage ( &buffer, &nm, &scopeName, &majorVersion, &minorVersion );
  me->RcvHelloMessage ( nm, scopeName, majorVersion, minorVersion );
  return 0;
}

int nmm_Microscope_Remote::handle_ScanrateParameter (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_float32 rate;

  me->decode_ScanrateParameter ( &buffer, &rate );
  me->RcvScanrateParameter ( rate );
  return 0;
}

int nmm_Microscope_Remote::handle_PidParameters (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_float32 P;
  vrpn_float32 I;
  ...
  me->decode_PidParameters ( &buffer, &P, &I, &D );
  me->RcvPidParameters ( P, I, D );
  return 0;
}

int nmm_Microscope_Remote::handle_ReportGridSize (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_int32 x;
  vrpn_int32 y;

  me->decode_ReportGridSize ( &buffer, &x, &y );
  me->RcvReportGridSize ( x, y );
  return 0;
}

int nmm_Microscope_Remote::handle_SetRegionClipped (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_float32 xmin;
  vrpn_float32 ymin;
  ...
  me->decode_SetRegionClipped ( &buffer, &xmin, &ymin, &xmax, &ymax );
  me->RcvSetRegionClipped ( xmin, ymin, xmax, ymax );
  return 0;
}

int nmm_Microscope_Remote::handle_ScanRange (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_float32 xmin;
  vrpn_float32 ymin;
  ...
  me->decode_ScanRange ( &buffer, &xmin, &ymin, &zmin, &xmax, &ymax, &zmax );
  me->RcvScanRange ( xmin, ymin, zmin, xmax, ymax, zmax );
  return 0;
}

int nmm_Microscope_Remote::handle_ReportScanAngle (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_float32 angle;

  me->decode_ReportScanAngle ( &buffer, &angle );
  me->RcvReportScanAngle ( angle );
  return 0;
}

int nmm_Microscope_Remote::handle_ReportSlowScan (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_int32 enabled;

  me->decode_ReportSlowScan ( &buffer, &enabled );
  me->RcvReportSlowScan ( enabled );
  return 0;
}

int nmm_Microscope_Remote::handle_Scanning (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_int32 on_off;

  me->decode_Scanning ( &buffer, &on_off );
  me->RcvScanning ( on_off );
  return 0;
}

int nmm_Microscope_Remote::handle_ScanDataset (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_int32 count;
  char name[64];
  ...
  me->decode_ScanDataset_header ( &buffer, &count );
  me->RcvScanDataset_header ( count );
  for (int lv_1 = 0; lv_1 < count; lv_1++) {
    me->decode_ScanDataset_body ( &buffer, &name, &units, &offset, &scale );
    me->RcvScanDataset_body ( name, units, offset, scale );
  }
  return 0;
}

int nmm_Microscope_Remote::handle_WindowLineData (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_int32 x;
  vrpn_int32 y;
  ...
  me->decode_WindowLineData ( &buffer, &x, &y, &dx, &dy, &reports, &fields, &sec, &usec, &data );
  me->RcvWindowLineData ( x, y, dx, dy, reports, fields, sec, usec, data );
  for (int lv_1 = 0; lv_1 < reports ; lv_1++ ) {
    delete [] data[lv_1];
  }
  delete [] data;
...
int nmm_Microscope_Remote::handle_PointDataset (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_int32 count;
  char name[64];
  ...
  me->decode_PointDataset_header ( &buffer, &count );
  me->RcvPointDataset_header ( count );
  for (int lv_1 = 0; lv_1 < count; lv_1++) {
    me->decode_PointDataset_body ( &buffer, &name, &units, &numSamples, &offset, &scale );
    me->RcvPointDataset_body ( name, units, numSamples, offset, scale );
  }
  return 0;
}

int nmm_Microscope_Remote::handle_ForceCurveData (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_float32 x;
  vrpn_float32 y;
  ...
  me->decode_ForceCurveData_header ( &buffer, &x, &y, &numSamples, &numHalfcycles, &sec, &usec );
  me->RcvForceCurveData_header ( x, y, numSamples, numHalfcycles, sec, usec );
  for (int lv_1 = 0; lv_1 < numSamples; lv_1++) {
    me->decode_ForceCurveData_body ( &buffer, &z, &d, &numHalfcycles );
    me->RcvForceCurveData_body ( z, d, numHalfcycles );
  delete [] d;
  }
  return 0;
}

int nmm_Microscope_Remote::handle_RelaxSet (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_int32 minTime;
  vrpn_int32 sepTime;

  me->decode_RelaxSet ( &buffer, &minTime, &sepTime );
  me->RcvRelaxSet ( minTime, sepTime );
  return 0;
}

int nmm_Microscope_Remote::handle_StartingToRelax (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_int32 sec;
  vrpn_int32 usec;

  me->decode_StartingToRelax ( &buffer, &sec, &usec );
  me->RcvStartingToRelax ( sec, usec );
  return 0;
}

int nmm_Microscope_Remote::handle_TopoFileHeader (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_int32 length;
  char *header;

  me->decode_TopoFileHeader ( &buffer, &length, &header );
  me->RcvTopoFileHeader ( length, header );
  delete [] header;
  return 0;
}

int nmm_Microscope_Remote::handle_SuspendCommands (void * userdata, vrpn_HANDLERPARAM p) {
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;

  me->RcvSuspendCommands (  );
  return 0;
}

int nmm_Microscope_Remote::handle_ResumeCommands (void * userdata, vrpn_HANDLERPARAM p) {
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;

  me->RcvResumeCommands (  );
  return 0;
}

int nmm_Microscope_Remote::handle_MaxSetpointExceeded (void * userdata, vrpn_HANDLERPARAM p) {
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;

  me->RcvMaxSetpointExceeded (  );
  return 0;
}

int nmm_Microscope_Remote::handle_InScanlineMode (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_int32 enabled;

  me->decode_InScanlineMode ( &buffer, &enabled );
  me->RcvInScanlineMode ( enabled );
  return 0;
}

int nmm_Microscope_Remote::handle_ScanlineData (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_float32 x;
  vrpn_float32 y;
  ...
  me->decode_ScanlineData ( &buffer, &x, &y, &z, &angle, &slope, &width, &resolution, &feedback_enabled, &checking_forcelimit, &max_force_setting, &max_z_step, &max_xy_step, &sec, &usec, &num_channels, &data );
  me->RcvScanlineData ( x, y, z, angle, slope, width, resolution, feedback_enabled, checking_forcelimit, max_force_setting, max_z_step, max_xy_step, sec, usec, num_channels, data );
  for (int lv_1 = 0; lv_1 < resolution ; lv_1++ ) {
    delete [] data[lv_1];
  }
  delete [] data;
  return 0;
...
int nmm_Microscope_Remote::handle_BeginFeelTo (void * userdata, vrpn_HANDLERPARAM p) {
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
...
int nmm_Microscope_Remote::handle_EndFeelTo (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_float32 x;
  vrpn_float32 y;
  ...
  me->decode_EndFeelTo ( &buffer, &x, &y, &numx, &numy, &dx, &dy, &orientation );
  me->RcvEndFeelTo ( x, y, numx, numy, dx, dy, orientation );
  return 0;
}

int nmm_Microscope_Remote::handle_InContactMode (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_float32 P;
  vrpn_float32 I;
  ...
  me->decode_InContactMode ( &buffer, &P, &I, &D, &setpoint );
  me->RcvInContactMode ( P, I, D, setpoint );
  return 0;
}

int nmm_Microscope_Remote::handle_InGuardedScanMode (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_float32 P;
  vrpn_float32 I;
  ...
  me->decode_InGuardedScanMode ( &buffer, &P, &I, &D, &setpoint, &fNormalX, &fNormalY, &fNormalZ, &fPlaneD, &fGuardDepth );
  me->RcvInGuardedScanMode ( P, I, D, setpoint, fNormalX, fNormalY, fNormalZ, fPlaneD, fGuardDepth );
  return 0;
}

int nmm_Microscope_Remote::handle_InTappingMode (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_float32 P;
  vrpn_float32 I;
  ...
  me->decode_InTappingMode ( &buffer, &P, &I, &D, &setpoint, &amplitude );
  me->RcvInTappingMode ( P, I, D, setpoint, amplitude );
  return 0;
}

int nmm_Microscope_Remote::handle_InOscillatingMode (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_float32 P;
  vrpn_float32 I;
  ...
  me->decode_InOscillatingMode ( &buffer, &P, &I, &D, &setpoint, &amplitude, &frequency, &input_gain, &drive_attenuation, &phase, &ampl_or_phase );
  me->RcvInOscillatingMode ( P, I, D, setpoint, amplitude, frequency, input_gain, drive_attenuation, phase, ampl_or_phase );
  return 0;
}

int nmm_Microscope_Remote::handle_InSewingStyle (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_float32 setpoint;
  ...
  me->decode_InSewingStyle ( &buffer, &setpoint, &bottomDelay, &topDelay, &pullBackDistance, &moveDistance, &moveRate, &maxDistanceToApproach );
  me->RcvInSewingStyle ( setpoint, bottomDelay, topDelay, pullBackDistance, moveDistance, moveRate, maxDistanceToApproach );
  return 0;
}

int nmm_Microscope_Remote::handle_InSpectroscopyMode (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_float32 setpoint;
  ...
  me->decode_InSpectroscopyMode ( &buffer, &setpoint, &startDelay, &zStart, &zEnd, &zPullback, &forceLimit, &distBetweenFC, &numPoints, &numHalfcycles, &sampleSpeed, &pullbackSpeed, &startSpeed, &feedbackSpeed, &averageNum, &sampleDelay, &pullbackDelay, &feedbackDelay );
  me->RcvInSpectroscopyMode ( setpoint, startDelay, zStart, zEnd, zPullback, forceLimit, distBetweenFC, numPoints, numHalfcycles, sampleSpeed, pullbackSpeed, startSpeed, feedbackSpeed, averageNum, sampleDelay, pullbackDelay, feedbackDelay );
  return 0;
}

int nmm_Microscope_Remote::handle_InDirectZControl (void * userdata, vrpn_HANDLERPARAM p) {
  const char * buffer = p.buffer;
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
  vrpn_float32 max_z_step;
  vrpn_float32 max_xy_step;
  ...
  me->decode_InDirectZControl ( &buffer, &max_z_step, &max_xy_step, &min_setpoint, &max_setpoint, &max_lateral_force, &freespace_normal_force, &freespace_lat_force );
  me->RcvInDirectZControl ( max_z_step, max_xy_step, min_setpoint, max_setpoint, max_lateral_force, freespace_normal_force, freespace_lat_force );
  return 0;
}

int nmm_Microscope_Remote::handle_InModMode (void * userdata, vrpn_HANDLERPARAM p) {
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
...
int nmm_Microscope_Remote::handle_InImgMode (void * userdata, vrpn_HANDLERPARAM p) {
  nmm_Microscope_Remote* me = (nmm_Microscope_Remote *) userdata;
...
void nmm_Microscope_Remote::RcvInTappingMode (
      vrpn_float32 P,
      vrpn_float32 I,
      vrpn_float32 D,
      vrpn_float32 setpoint,
...
void nmm_Microscope_Remote::RcvInOscillatingMode (
      vrpn_float32 P,
      vrpn_float32 I,
      vrpn_float32 D,
      vrpn_float32 setpoint,
...
void nmm_Microscope_Remote::RcvInContactMode (
      vrpn_float32 P,
      vrpn_float32 I,
      vrpn_float32 D,
      vrpn_float32 setpoint
...
void nmm_Microscope_Remote::RcvInGuardedScanMode (
      vrpn_float32 P,
      vrpn_float32 I,
      vrpn_float32 D,
      vrpn_float32 setpoint,
...
void nmm_Microscope_Remote::RcvInDirectZControl (
      vrpn_float32 max_z_step,
      vrpn_float32 max_xy_step,
      vrpn_float32 min_setpoint,
      vrpn_float32 max_setpoint,
...
void nmm_Microscope_Remote::RcvInSewingStyle (
      vrpn_float32 setpoint,
      vrpn_float32 bottomDelay,
      vrpn_float32 topDelay,
      vrpn_float32 pullBackDistance,
...
void nmm_Microscope_Remote::RcvInSpectroscopyMode (
      vrpn_float32 setpoint,
      vrpn_float32 startDelay,
      vrpn_float32 zStart,
      vrpn_float32 zEnd,
...
void nmm_Microscope_Remote::RcvInModMode (
) {
...
void nmm_Microscope_Remote::RcvInImgMode (
) {
...
void nmm_Microscope_Remote::RcvStartingToRelax (
      vrpn_int32 sec,
      vrpn_int32 usec
) {
//    int32 sec
...
void nmm_Microscope_Remote::RcvRelaxSet (
      vrpn_int32 minTime,
      vrpn_int32 sepTime
) {
//    int32 minTime
...
void nmm_Microscope_Remote::RcvWindowLineData (
      vrpn_int32 x,
      vrpn_int32 y,
      vrpn_int32 dx,
      vrpn_int32 dy,
...
void nmm_Microscope_Remote::RcvPointResultData (
      vrpn_float32 x,
      vrpn_float32 y,
      vrpn_int32 sec,
      vrpn_int32 usec,
...
void nmm_Microscope_Remote::RcvScanning (
      vrpn_int32 on_off
) {
//    int32 on_off
...
void nmm_Microscope_Remote::RcvScanRange (
      vrpn_float32 xmin,
      vrpn_float32 ymin,
      vrpn_float32 zmin,
      vrpn_float32 xmax,
...
void nmm_Microscope_Remote::RcvReportScanAngle (
      vrpn_float32 angle
) {
//    float32 angle
  float newangle;
...
void nmm_Microscope_Remote::RcvSetRegionClipped (
      vrpn_float32 xmin,
      vrpn_float32 ymin,
      vrpn_float32 xmax,
      vrpn_float32 ymax
...
void nmm_Microscope_Remote::RcvReportSlowScan (
      vrpn_int32 enabled
) {
//    int32 enabled
...
void nmm_Microscope_Remote::RcvHelloMessage (
      char nm[4],
      char scopeName[SPM_NAME_LENGTH],
      vrpn_int32 majorVersion,
      vrpn_int32 minorVersion
...
void nmm_Microscope_Remote::RcvScanDataset_header (
      vrpn_int32 count
) {
//    int32 count
    count = count;
...
void nmm_Microscope_Remote::RcvScanDataset_body (
      char name[64],
      char units[64],
      vrpn_float32 offset,
      vrpn_float32 scale
...
void nmm_Microscope_Remote::RcvPointDataset_header (
      vrpn_int32 count
) {
//    int32 count
    count = count;
...
void nmm_Microscope_Remote::RcvPointDataset_body (
      char name[64],
      char units[64],
      vrpn_int32 numSamples,
      vrpn_float32 offset,
...
void nmm_Microscope_Remote::RcvPidParameters (
      vrpn_float32 P,
      vrpn_float32 I,
      vrpn_float32 D
) {
...
void nmm_Microscope_Remote::RcvScanrateParameter (
      vrpn_float32 rate
) {
//    float32 rate
  if (state.acquisitionMode == MODIFY) {
...
void nmm_Microscope_Remote::RcvReportGridSize (
      vrpn_int32 x,
      vrpn_int32 y
) {
//    int32 x
...
void nmm_Microscope_Remote::RcvTopoFileHeader (
      vrpn_int32 length,
      char *header
) {
//    int32 length
...
void nmm_Microscope_Remote::RcvForceCurveData_header (
      vrpn_float32 x,
      vrpn_float32 y,
      vrpn_int32 numSamples,
      vrpn_int32 numHalfcycles,
...
void nmm_Microscope_Remote::RcvForceCurveData_body (
      vrpn_float32 z,
      vrpn_float32 *d
      , vrpn_int32 numHalfcycles
) {
...
void nmm_Microscope_Remote::RcvInScanlineMode (
      vrpn_int32 enabled
) {
//    int32 enabled   
  if (enabled){
...
// updates data object
void nmm_Microscope_Remote::RcvScanlineData (
      vrpn_float32 x,
      vrpn_float32 y,
      vrpn_float32 z,
      vrpn_float32 angle,
...
void nmm_Microscope_Remote::RcvMaxSetpointExceeded (
) {
...
void nmm_Microscope_Remote::RcvBeginFeelTo (
) {
  accumulatePointResults(VRPN_TRUE);
}
void nmm_Microscope_Remote::RcvEndFeelTo (
      vrpn_float32 x,
      vrpn_float32 y,
      vrpn_int32 numx,
      vrpn_int32 numy,
