...
vrpn_Ohmmeter::vrpn_Ohmmeter(char * /*name*/, vrpn_Connection *c)
{
  ...
  //servicename = vrpn_copy_service_name(name);
    ...
    if (c) {
	//my_id = d_connection->register_sender(servicename);

	channelset_m_id = c->register_message_type("Channel Settings");
	measure_m_id = c->register_message_type("Resistance");
	setchannel_m_id = c->register_message_type("Set Channel");
    }
    timestamp.tv_sec = 0;
    timestamp.tv_usec = 0;
    channel = 0;
              ...
              vrpn_Ohmmeter(name,c)
{
    ...
    if (d_connection)
	if (d_connection->register_handler(setchannel_m_id,
	    handle_change_message, this, d_myId))
...
void vrpn_Ohmmeter_ORPX2::mainloop(void) {
	...
	if (d_connection) {
		...
		if (d_connection->pack_message(len, timestamp, measure_m_id,
						...
						const vrpn_SETOHMCB &info)
{
	...
	if (me->d_connection){	//send message saying that channel was set
		len = me->encode_channelset_to(outputbuf);
		if (me->d_connection->pack_message(len, me->timestamp, 
		    me->channelset_m_id, 
		    me->d_myId, outputbuf,vrpn_CONNECTION_RELIABLE)) {
		      fprintf(stderr, "ORPX2: can't write message: tossing\n");
		}
	}
...
vrpn_Ohmmeter_Server::vrpn_Ohmmeter_Server(char *name, vrpn_Connection *c,
        vrpn_float32 hz):
        nmb_Device_Server(name,c),
        vrpn_Ohmmeter(name,c)
{
...
void vrpn_Ohmmeter_Server::mainloop(void) {
    struct timeval current_time;
    char msgbuf[1000];
    vrpn_int32 len;
...
void vrpn_Ohmmeter_Server::handle_channel_set(void *userdata,
                                                const vrpn_SETOHMCB &info)
{

        vrpn_Ohmmeter_Server *me = (vrpn_Ohmmeter_Server *)userdata;
    vrpn_int32 i;
        vrpn_int32 len;
        char outputbuf[1000];
        struct timeval current_time;
...
int vrpn_Ohmmeter_Server::handle_change_message(void *userdata,
                                                        vrpn_HANDLERPARAM p)
{
    vrpn_Ohmmeter_Server *me = (vrpn_Ohmmeter_Server *)userdata;
    vrpn_float64 *params = (vrpn_float64 *)(p.buffer);
...
int vrpn_Ohmmeter_Server::register_change_handler(void *userdata,
        vrpn_SETOHMCHANGEHANDLER handler)
{
    vrpn_ORPXCHANGELIST *new_entry;
...
int vrpn_Ohmmeter_Server::unregister_change_handler(void *userdata,
        vrpn_SETOHMCHANGEHANDLER handler)
{
    vrpn_ORPXCHANGELIST *victim, **snitch;
...
vrpn_Ohmmeter_Remote::vrpn_Ohmmeter_Remote(char *name, vrpn_Connection *c):
    nmb_Device_Client(name, (c==NULL ? vrpn_get_connection_by_name(name) : c)),
    vrpn_Ohmmeter(name, d_connection)
{
    ...
    if (d_connection->register_handler(measure_m_id, handle_measurement_message,
	this, d_myId)) {
	    fprintf(stderr, 
    ...
    if (d_connection->register_handler(channelset_m_id, handle_ohmset_message,
	this, d_myId)) {
	    fprintf(stderr,
	...
	vrpn_float64 volt, vrpn_float64 r_min, vrpn_float64 filt)
{
	...
	if (d_connection->pack_message(len,timestamp,setchannel_m_id,
		d_myId, msgbuf, vrpn_CONNECTION_RELIABLE)) {
