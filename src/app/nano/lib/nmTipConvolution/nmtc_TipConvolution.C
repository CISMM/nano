...
nmtc_TipConvolution::nmtc_TipConvolution(nmg_Graphics *g, nmb_ImageList *im):
  ...
  //d_convlutionEnabled("conv_window_open" , 0),
...

{
  ...
  // d_convolutionEnabled.addCallback
  //(handle_convolutionEnabled_change, (void *)this);
...
{
   printf("nmtc_TipConvolution::CreateConvolutionImage\n");
...
// the user has given a name to to the output image
  ...
  if((!Surface) ||(!Tip) || (strlen(d_resultImageName.string())) == 0)
   ...
   printf("allocatte image.This is a test");
 ...
 for (i=0; i < (Surface->width());i++){
   for (j=0; j < (Surface->height());j++){
     ...
     // current surface location.
      ...
      distance = (Surface->getValue(i,j)) + (Tip->getValue(tipcentx,tipcenty));
	...
	x= i+k-tipcentx;
	  ...
	  // up carving to the same height as the Surface was.
        ...
        if((x >= 0 && x < Surface->width()) && (y >=0 && y<Surface->height())){          //Beacause we are using simulated tips created in Photoshop to 
	  ...
	  // actual tip to do most of the carving.  Therefore, to eliminate 
          ...
          // by the tip in the plane.
            if (Tip->getValue(k,l) != 0){
                double carved_z = distance - Tip->getValue(k,l);
