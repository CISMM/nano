...
int main(int argc, char **argv) {
	...
	if (!init_Tk()){
	  ...
	  // Make a frame for the ohmmeter controls.
	  sprintf(command, "toplevel .french_ohmmeter");
	  TCLEVALCHECK(tk_control_interp, command);
	  ...
	  sprintf(command, "button .french_ohmmeter.quit_button -text \"QUIT\""
			" -command \"set quit 1\" -bg red");
	  TCLEVALCHECK(tk_control_interp, command);
	  sprintf(command, "wm protocol .french_ohmmeter WM_DELETE_WINDOW"
				" {.french_ohmmeter.quit_button invoke}");
	  TCLEVALCHECK(tk_control_interp, command);
	  sprintf(command, "pack .french_ohmmeter.quit_button -side right -anchor e -ipadx 3m");
			//-in .french_ohmmeter -side right -anchor e -ipadx 3m");
	  TCLEVALCHECK(tk_control_interp, command);
