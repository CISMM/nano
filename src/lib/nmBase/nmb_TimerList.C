...
void nmb_TimerList::blockList (vrpn_int32 sN) {
  nmb_Timestamp * ts;
  ...
  fprintf(stderr, "nmb_TimerList::blockList:  %d not in list.\n", sN);
}

void nmb_TimerList::unblockList (vrpn_int32 sN) {
  nmb_Timestamp * ts;
...
void nmb_TimerList::report (void) {
  ...
  fprintf(stderr, "Total elapsed time:  %d sec %d usec.\n",
          d_totalTimeComplete.tv_sec, d_totalTimeComplete.tv_usec);
  fprintf(stderr, "Total timestamps taken:  %d.\n",
          d_totalTimestampsComplete);

  meantime = vrpn_TimevalScale (d_totalTimeComplete,
                                1.0 / d_totalTimestampsComplete);
  fprintf(stderr, "Mean elapsed time per timestamp:  %d sec %d usec.\n",
