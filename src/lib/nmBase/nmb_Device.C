...

{
  ...
  if (d_connection) {
    ...
    d_connection->register_handler(d_DroppedLastConnection_type,
                                   ...
                                   this);
    // HACK: If we're already connected before we register the
                                           ...
                                           vrpn_int32 type)
{
  ...
  // Useful if we are viewing a file
  if (d_redundancy) {
      retval = d_redundancy->pack_message(len, now, type, d_myId, (char *) buf,
                                      ...
                                      vrpn_int32 type)
{
  ...
  if (!d_useBuffer) {
    retval = dispatchMessageNoBuffer(len, buf, type);
  } else {
    d_bufferSemaphore.p();
    retval = bufferMessage(len, buf, type);
    if (!retval) {
      d_messageBufferTail->isRedundant = VRPN_FALSE;
    }
    d_bufferSemaphore.v();
  }
  return retval;
                                      ...
                                      vrpn_int32 type)
{
  ...
  if (!d_useBuffer) {
    retval = dispatchRedundantMessageNoBuffer(len, buf, type);
  } else {
    d_bufferSemaphore.p();
    retval = bufferMessage(len, buf, type);
    if (!retval) {
